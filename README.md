...
- `RecallBridge` — tells a local [NX Recall](https://github.com/nerdrx/nx-recall) which Discord user was talking, and when. Recall records the mixed call off your speakers and has to guess whose voice is whose; Discord's client already knows, so this hands over the speaking edges. No messages, 127.0.0.1 only. There's a second, **default-off** option that sends each person's *audio* separately — on Vesktop and the web client every remote user arrives as their own `MediaStream`, so Recall can record and name them individually and never has to guess at all. That one takes everybody's voice out of the client, which is why it's off and why you also have to turn `[truth] audio` on in recalld. The Discord desktop client can't do it (voice is decoded inside a native module) and the toolbox says so rather than pretending.
